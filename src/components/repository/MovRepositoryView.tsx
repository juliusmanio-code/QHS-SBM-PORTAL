...
export const MovRepositoryView: React.FC<MovRepositoryViewProps> = ({
  ...
  const { movRecords, dimensions, indicators, currentSchoolYear, deleteMov } = useSbmData();
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-right whitespace-nowrap">
                        <div className="inline-flex items-center space-x-1.5">
                          ...
                          <button
                            id={`delete-mov-table-${mov.id}`}
                            onClick={() => setMovToDelete(mov)}
                            className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer"
                            title="Delete MOV Document"
                          >
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  {canReviewMov(mov.dimensionId) && (
                    <button
                      onClick={() => onReviewMov(mov)}
                      className="px-2.5 py-1 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-medium cursor-pointer"
                    >
                      Review
                    </button>
                  )}
                  <button
                    id={`delete-mov-grid-${mov.id}`}
                    onClick={() => setMovToDelete(mov)}
        ...
        >
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4 text-slate-800 animate-in fade-in duration-150">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Delete MOV Document</h3>
                <p className="text-xs text-slate-500">Remove evidence file from repository</p>
              </div>
            </div>
            ...
            <p className="text-xs text-rose-600 bg-rose-50 p-2.5 rounded-lg border border-rose-200">
              Are you sure you want to delete this file? This action will remove the document from the school repository.
            </p>

            <div className="flex items-center justify-end space-x-2 pt-2 border-t border-slate-100">
              <button
                type="button"
                id="cancel-repo-delete-btn"
